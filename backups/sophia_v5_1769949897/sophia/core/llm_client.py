            ...
            for turn in range(max_turns):
                ...
                for part in model_content.parts:
                    if part.function_call:
                        ...
                        all_results["tool_calls"].append(call_info)
                        
                        # We need to execute the tool to feed it back!
                        # But GeminiClient shouldn't have the Hand. 
                        # We return the calls so the caller can execute and we resume.
                        # Wait, a better pattern: pass an executor callback.
                        return all_results # Return first batch so main loop can execute.

                    if part.text:
                        all_results["text"] += part.text

                if not found_tool_call:
                    break # Model finished its thought
