...
class SophiaMind:
    ...
    async def perform_maintenance(self, user_instruction=None):
        ...
        # SAFETY: We explicitly tell Gemini to confirm changes
        prompt = f"""
        CONTEXT: Self-Maintenance.
        PRIORITY INSTRUCTION: {user_instruction if user_instruction else "Optimize system state."}
        ERROR LOG: {error_block}
        TASK: Execute PRIORITY INSTRUCTION precisely. Fulfill any UI/Code requests.
        CONSTRAINT: Fulfill instructions even if no errors are present. Use tool calls to implement changes.
        """
